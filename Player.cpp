...
{ 
	...
	if (behavior_ == Behavior::kAttack) 
	{
		...
		case AttackPhase::kRecovery:
			modelAttack->Draw(worldTransformAttack, *camera_);
			break;
		}
	}
}
...
void Player::BehaviorAttackInitialize() 
{
	...
	velocity_ = {};
	// ためるフェーズから始める
	attackPhase = AttackPhase::kAnticipation;
}
...
void Player::BehaviorAttackUpdate() 
{
	...
	default: 
	{
		...
		// 前進動作へ移行
		if (attackParameter >= kAnticipationTime) 
		{
			attackPhase = AttackPhase::kAction;
			attackParameter = 0; // パラメータをリセット
		}
		break;
	}
	case AttackPhase::kAction: 
	{
		...
		float t = static_cast<float>(attackParameter) / kActionTime;
		worldTransform_.scale_.z = EaseOut(0.3f, 1.3f, t);
		worldTransform_.scale_.y = EaseIn(1.6f, 0.7f, t);

		// 復帰動作へ移行
		if (attackParameter >= kActionTime) 
		{
			attackPhase = AttackPhase::kRecovery;
			attackParameter = 0; // パラメータをリセット
		}
		break;
	}
	...
	{
		velocity = {};
		float t = static_cast<float>(attackParameter) / kRecoveryTime;
		worldTransform_.scale_.z = EaseOut(1.3f, 1.0f, t);
		worldTransform_.scale_.y = EaseOut(0.7f, 1.6f, t);

		// 通常行動に戻る
		if (attackParameter >= kRecoveryTime) 
		{
	...
	worldTransformAttack.translation_ = worldTransform_.translation_;
	worldTransformAttack.rotation_ = worldTransform_.rotation_;
}
...
void Player::OnCollision(const FallingBlock* FallingBlock)
{
	(void)FallingBlock;
